...
class FrappeAPI:
    ...
    def _setup_auth(self, auth_config):
        if 'api_key' in auth_config and 'api_secret' in auth_config:
            ...
            logging.warning("Keine Frappe API-Token in der Konfiguration gefunden. Es wird ohne Authentifizierung versucht.")
    ...
    def get_data(self, endpoint, params=None):
        try:
            ...
            response.raise_for_status()
            logging.info(f"Daten erfolgreich von {endpoint} abgerufen.")
            return response.json()
        except requests.exceptions.RequestException as e:
